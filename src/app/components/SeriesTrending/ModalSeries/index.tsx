...
function ModalSeries({ isOpen, onClose, props }: any) {
  ...
  return (
    <div className="fixed w-full px-5 h-full text-white py-2 inset-0 flex items-center justify-center z-50">
      <div className="bg-black w-full h-auto px-5 rounded-lg relative">
        <button className="absolute top-2 right-2 text-xl" onClick={onClose}>
          &times;
        </button>
        <h2 className="text-3xl font-bold m-5">{props?.title}</h2>
        <div className="relative w-full min-w-44 h-auto flex flex-col justify-center items-start">
          <Image
            src={`https://image.tmdb.org/t/p/original/${props?.poster}`}
            alt={props?.poster}
            className="maskModal w-full max-w-96 rounded-3xl"
            width={0}
            height={0}
            sizes="100vw"
            placeholder="blur"
          ...
          />
          <div className="flex py-2 px-2 flex-row gap-2">
          ...
          <div className="w-full gap-2 bottom-0 py-3 px-2 flex flex-col">
            <span className="font-semibold">{props?.title}</span>
            <span className="font-extralight">{props?.overview}</span>
          </div>
        </div>
        <Swiper
          spaceBetween={0}
